...
func Load() Config {
	return Config{
		...
		RedisHost:           getEnv("HOST", "localhost"),
		RedisPort:           getEnv("PORT", "6379"),
		RedisPassword:       getEnv("PASSWORD", "my_secret_password"),
		RedisTTL:            time.Duration(getEnvInt("TTL_HOURS", 168)) * time.Hour,
